...
const TeacherAuthorization = () => {
  ...
  const handleApprove = () => {
    ...
    students.sort((a, b) => a.name.localeCompare(b.name, 'ko-KR')).forEach((student, index) => {
        ...
        <tbody>
          {requests.map((request, index) => (
            <tr key={index}>
              ...
              <td>{request.date}</td>
              <td>
                <button className={styles.approveButton} onClick={() => handleApproveClick(request)}>승인</button>
                <button className={styles.rejectButton} onClick={() => handleRejectClick(request)}>거절</button>
              </td>
            </tr>
          ))}
        </tbody>
